...
void solve(){
    ...
    dsu.init(n);
    lp0(i,n){
        if(i+k<n){dsu.unite(i,i+k);}
        if(i+k+1<n){dsu.unite(i,i+k+1);}
    }
    vvc set_s(n),set_t(n);
    lp0(i,n){
        set_s[dsu.get(i)].pb(s1[i]); set_t[dsu.get(i)].pb(s2[i]);
    }
    lp0(i,n){
        if(!set_s[i].empty()){
            sort(set_s[i].begin(),set_s[i].end());
            sort(set_t[i].begin(),set_t[i].end());
            if(set_s[i]!=set_t[i]){flag=0;}
        }
    }
    if(flag){sayy;}
    else{sayn;}
