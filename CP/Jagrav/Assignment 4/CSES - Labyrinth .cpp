...
void solve(){
    ...
    ll n,m; cin>>n>>m; queue<pll> q;
    vvc g(n,vc(m));
    vector<vector<bool>> vis(n,vector<bool>(m));
    vvc path(1000,vc(1000));
    pll f1,f2;
    lp0(i,n){
        lp0(j,m){
            cin>>g[i][j];
    ...
    while(!q.empty()){
        pll x=q.front(); q.pop();
        lp0(i,4){
            pll y={x.ff+dx[i],x.ss+dy[i]};
            if(y.ff<0||y.ff>=n||y.ss<0||y.ss>=m) continue;
            if(g[y.ff][y.ss]=='#') continue;
            if(vis[y.ff][y.ss]) continue;
            vis[y.ff][y.ss]=1;
            path[y.ff][y.ss]=i;
            q.push(y);
        }
    ...
    if(vis[f2.ff][f2.ss]){
        ...
        vll pt;
        while(f2!=f1){
            ll p=path[f2.ff][f2.ss];
            pt.pb(p);
            f2={f2.ff-dx[p],f2.ss-dy[p]};
        }
    reverse(pt.begin(),pt.end());
 
    o1(pt.size());
    for(char c:pt){cout<<step[c];}
    }
    else{sayn;}
